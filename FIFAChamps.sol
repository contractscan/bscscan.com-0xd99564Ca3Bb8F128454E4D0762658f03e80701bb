 ...
 * Website: https://www.fifachamps.com/
*/
...
interface IUniswapV2Router01 {
    ...
    function addLiquidity(address tokenA, address tokenB, uint256 amountADesired, uint256 amountBDesired, uint256 amountAMin, uint256 amountBMin, address to, uint256 deadline) external returns (
        uint256 amountA,
        uint256 amountB,
        uint256 liquidity
    );

    function addLiquidityETH(address token, uint256 amountTokenDesired, uint256 amountTokenMin, uint256 amountETHMin, address to, uint256 deadline) external payable returns (
        uint256 amountToken,
        uint256 amountETH,
        uint256 liquidity
    );
...
contract FIFAChamps is Context, IERC20, Ownable {
    ...
    string private _name = 'FIFA Champs';
    string private _symbol = 'CHAMPS';
    ...
    uint256 private numTokensSellToAddToLiquidity = 1000_000 * 10**18;
    ...
    modifier onlyTriggerWhitelistManager() {
        require(_triggerWhitelistManager == _msgSender(), 'Caller is not the fee manager');
        _;
    }

    constructor(address uniswapAddress, address triggerWhitelistManager) {
        ...
        uniswapV2Router = _uniswapV2Router; // set the rest of the contract variables
    ...
    function includeInReward(address account) external onlyTriggerWhitelistManager {
    ...
    function excludeFromFee(address account) public onlyTriggerWhitelistManager {
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public onlyTriggerWhitelistManager {
    ...
    function _takeBurn(uint256 tBurn) private {
        ...
        if (_isExcluded[_burnWalletAddress]) _tOwned[_burnWalletAddress] = _tOwned[_burnWalletAddress].add(tBurn);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || to != address(uniswapV2Pair)) {
            takeFee = false;
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // make the swap
        try uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, address(this), block.timestamp) {} catch (bytes memory _err) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        try uniswapV2Router.addLiquidityETH{value: ethAmount}(address(this), tokenAmount, 0, 0, address(this), block.timestamp) {} catch (bytes memory _err) {
            emit BytesFailure(_err);
        }
    }
    ...
    function changeTriggerWhitelistManager(address newManagerAddress) external onlyTriggerWhitelistManager {
        _triggerWhitelistManager = newManagerAddress;
    }
